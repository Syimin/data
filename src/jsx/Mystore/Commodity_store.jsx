...
import DeleteCButton_store from './DeleteCButton_store'
...
const UserCard = (props)=>{
    ...
    return(
        <div>
            ...
            />
            <br/>
            <center>{author.storename}</center>
        </div>
    )
}
...
const ContentCard = (props)=>{
    ...
    const collect4 = async (target)=>{
        try {
            ...
            const res = await fetch(`${HOST}:${PORT}/api/users/collects4/${target}`,{
                method:"PATCH",
    ...
    const uncollect4 = async (target)=>{
        try {
            ...
            const res = await fetch(`${HOST}:${PORT}/api/users/uncollects4/${target}`,{
                method:"PATCH",
                headers:{"Content-Type":"application/json"},
                body:JSON.stringify(body)
            })
    ...
    const button = user.collects4.includes(floor._id)
    ?<span onClick={()=>uncollect4(floor._id)}><StarFilled style={{fontSize:"24px",color:"orangered"}}/></span>
    :<span onClick={()=>collect4(floor._id)}><StarOutlined style={{fontSize:"24px",color:"orangered"}}/></span>
    ...
    return(
        <div>
            ...
            ?
            <>
            {button}<span style={{fontSize:"20px"}}>{floor.dianzans_storecommodity.length}</span>
            </>
            :null
            }
            ...
            <p className="d-flex justify-content-end text-muted">
                {hideDeleteCButton===false ? null : <DeleteCButton_store cid={floor._id}/>}
            </p>
            )
            : null
            }
            ...
            <hr/>
            {auth && (floor.author === floor.author.username)
            ?
            (  
            ...
            :
            (<p className="d-flex justify-content-end m-0">
                <small>{floor.author.description}</small><br/>
                <span style={{fontSize:"14px",color:"gray"}}><WarningOutlined />举报</span>
            </p>)
            }
...
export default Commodity_store