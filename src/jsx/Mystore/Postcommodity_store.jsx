...
const Postcommodity_store = (props)=>{
  const {history} = props
  ...
  const onFinish = async (values) => {
    ...
    const storage2 = await localStorage.getItem(DOMAIN2)//storage是字符串形式
    const {storename,s_token} = JSON.parse(storage2)//要解构就得先将storage转换成json格式
    const { title, content,category,price} = values;
    const body = { title, content,category,price, filelist ,username,token,storename,s_token};
    postcommodity(body);
  };
  ...
  //上传的图标样式
    ...
    return (
      <div className="pt-4">
        <h3 style={{textAlign:"center"}}>发布商铺商品</h3>
        <Form
        {...layout}
        name="basic"
        onFinish={onFinish}>
...
export default Postcommodity_store