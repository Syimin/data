...
const PostLikediscussesButton = (props)=>{
    const {loadLikediscusses} = props
    const [show,setShow] = useState(false)//默认为关闭状态
    const showModal = ()=> setShow(true)//开启函数
    const closeModal = ()=> setShow(false)//关闭函数
    const post = async (body)=>{
        try {
            const res = await fetch(`${HOST}:${PORT}/api/likediscusses`,{ //根据商品发布讨论
                method:"POST",
    ...
    return(
        <div>
            <Button variant="warning" size="sm" onClick={()=>showModal()}>
                发起分享
            </Button>
            <Modal show={show} onHide={()=>closeModal()}>
                <Modal.Header closeButton>
                    <Modal.Title>种草帖</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <PostForm/>
                </Modal.Body>
...
export default PostLikediscussesButton