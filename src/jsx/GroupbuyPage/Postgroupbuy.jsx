...
const Postgroupbuy = (props)=>{
  ...
  const onFinish = async (values) => {
    ...
    const { title, content,category,motoprice,price,number} = values;
    const body = { title, content,category,motoprice,price,number, filelist ,username,token};
    postgroupbuy(body);
  };
  ...
  //上传图片样式
    ...
    return (
      <div className="pt-4">
        <h3 style={{textAlign:"center"}}>发起团购</h3>
      <Form
      {...layout} 
      name="basic"
      onFinish={onFinish}>
        ...
        <Form.Item
          label="团购价格"
          name="price"
          rules={[{ required: true, message: "请输入商品团购价" }]}
        >
          <Input />
        </Form.Item>
        {/* <Form.Item
