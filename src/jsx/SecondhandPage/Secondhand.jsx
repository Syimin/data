...
const ContentCard = (props)=>{
    ...
    const collect2 = async (target)=>{
        try {
            ...
            const res = await fetch(`${HOST}:${PORT}/api/users/collects2/${target}`,{
                method:"PATCH",
    ...
    const uncollect2 = async (target)=>{
        try {
            ...
            const res = await fetch(`${HOST}:${PORT}/api/users/uncollects2/${target}`,{
                method:"PATCH",
                headers:{"Content-Type":"application/json"},
                body:JSON.stringify(body)
            })
    ...
    const button = user.collects2.includes(floor._id)
    ?<span onClick={()=>uncollect2(floor._id)}><StarFilled style={{fontSize:"24px",color:"orangered"}}/></span>
    :<span onClick={()=>collect2(floor._id)}><StarOutlined style={{fontSize:"24px",color:"orangered"}}/></span>
    ...
    const putin = async (target)=>{
        try {
            ...
            const res = await fetch(`${HOST}:${PORT}/api/shoppingcarts2/putin/${target}`,{
                method:"PATCH",
                headers:{"Content-Type":"application/json"},
                body:JSON.stringify(body)
            })
    ...
    return(
        <div>
            ...
            ?
            <>
            {button}<span style={{fontSize:"20px"}}>{floor.dianzans_secondhand.length}</span>
            </>
            :
            null
            }
...
const GPart = (props)=>{
    ...
    return(
        <Media className="rounded border p-3 mb-3 mt-0">
            <UserCard author={secondhand.author}/>
            <Media.Body>
                <ContentCard floor={secondhand} hideDeleteCButton={false}/>
            </Media.Body>
        </Media>
    )
}
...
const Secondhand = (props)=>{
    const {secondhand,loadSecondhand,comments} = props
    useEffect(()=>{
        loadSecondhand();
    },[])
