...
const Postsecondhand = (props)=>{
  ...
  const postsecondhand = async (body)=>{
    try {
        const res = await fetch(`${HOST}:${PORT}/api/secondhands`, {
        method: "POST",
  ...
  const onFinish = async (values) => {
    ...
    const { title, content,category,price,number} = values;
    const body = { title, content,category,price,number, filelist ,username,token};
    postsecondhand(body);
  };
  ...
  //上传图片样式
    ...
    return (
      <div className="pt-4">
        <h3 style={{textAlign:"center"}}>闲置二手</h3>
      <Form 
      {...layout}
      name="basic" 
      onFinish={onFinish}>
        <Form.Item
          label="二手商品标题"
          name="title"
          rules={[{ required: true, message: "请输入二手商品标题" }]}
        >
          <Input />
        </Form.Item>
        <Form.Item
          label="二手商品内容"
          name="content"
          rules={[{ required: true, message: "请输入二手商品内容" }]}
        >
          <Input />
        </Form.Item>
        <Form.Item
          label="二手商品类型"
          name="category"
          rules={[{ required: true, message: "请输入二手商品类型" }]}
        >
          <Input />
        </Form.Item>
        <Form.Item
          label="二手商品价格"
          name="price"
          rules={[{ required: true, message: "请输入二手商品价格" }]}
        >
          <Input />
        </Form.Item>
        <Form.Item
          label="商品几成新"
          name="number"
          rules={[{ required: true, message: "请输入商品几成新" }]}
        >
          <Input />
        </Form.Item >
