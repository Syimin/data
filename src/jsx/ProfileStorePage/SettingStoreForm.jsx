...
const SettingStoreForm = (props)=>{
    ...
    const loadSettings = async ()=>{//获取用户当前信息接口
        try {
            ...
            const storage2 = await localStorage.getItem(DOMAIN2)//storage是字符串形式，读取用getItem,从DOMAIN读取
            const {storename,s_token} = JSON.parse(storage2)//要解构就得先将storage的字符串格式转换成json格式
            const res = await fetch(`${HOST}:${PORT}/api/stores/${sid}`,{//调用接口
                method:"GET"
            })
    ...
    const handleSettings = (e)=>{
        ...
        const form = document.forms.settingstoreForm
    ...
    return(
        //file表示提交文件类型
        <Form id="settingstoreForm">
            <Form.Group controlId="description">
                <Form.Label>商铺描述</Form.Label>
                <Form.Control type="text" placeholder="请输入商铺描述" defaultValue={settings.description}/>
            </Form.Group>
            <Form.Group controlId="uploader">
                ...
                <img
                    ...
                    alt="头像"
                />
                <Form.Control type="file" placeholder="请上传商铺头像"/>
            </Form.Group>
...
export default SettingStoreForm